...
function FilterIcon({table}) {
  ...
  const handleFilterChange =useCallback( (e, columnId,selectAll) => {
  ...
  const closeDropdowns = () => {
    ...
    const columnFiltersObject = useMemo(()=>columnFilters.reduce((acc , column)=>{
  ...
  return (
    <div>
          <button className='filterIcon' onClick={() => setDropdownOpen(prev => {
            if(prev){
              setSelectedColumnId(null)
            }
            return !prev
              ...
              <ul>
                <li>
                    <input
                      ...
                      onChange={e => handleFilterChange(e, selectedColumnId,true)}
                ...
                {uniqueSelectedColumnRows?.map((rowValue, index) => {
                    const columnFiltersSet = new Set(columnFiltersObject[selectedColumnId]  || [])
                  return (
                  <li key={index}>
                    <input
                      type="checkbox"
